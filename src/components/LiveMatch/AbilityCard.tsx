...
export function AbilityCard({ ability }: { ability: Ability }) {
  ...
  return (
    <div className="bg-surface border border-background-border rounded-xl p-4 flex gap-4 shadow-xl">
      <img
        src={ability.icon}
        alt={ability.title}
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            <span className="text-sm font-bold text-white uppercase tracking-wide">
              {ability.title}
              {typeLabel && (
                <span className="text-zinc-400 font-semibold"> [{typeLabel}]</span>
              )}
            </span>
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              {ability.tags.map(tag => (
                <span
                  key={tag}
                  className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 border border-zinc-700 rounded px-1.5 py-0.5"
                >
                  {tag}
                </span>
              ))}
