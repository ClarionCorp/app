...
local function BuildRoster()
    ...
    for _, ps in ipairs(TrackedStates) do
        local ok, entry = pcall(function()
            ...
            local role = (okGoalie and isGoalie) and "Goalie" or "Forward"
            ...
            local kos = playerId ~= "" and (KoCounts[playerId] or 0) or 0
            return {
                name = name, team = team, playerId = playerId, role = role,
                charId = charId, charName = charName, trainings = trainings,
...
local function WriteRoster(ModName, PLAYERS_FILE)
    ...
    if snapshot == LastSnapshot and not pingChanged then return end
    ...
    if not f then print(string.format("[%s] Failed to write players file", ModName)) return end
    f:write(body)
    f:close()
    print(string.format("[%s] Roster: Updated %d players", ModName, #roster))
end

function Module.Init(ModName, OUT_DIR)
    local PLAYERS_FILE = OUT_DIR .. "\\players.json"
    print(string.format("[%s] Writing roster to: %s", ModName, PLAYERS_FILE))
    ...
    pcall(function()
        ...
        print(string.format("[%s] MatchPhaseChanged hook registered (roster)", ModName))
    end)
