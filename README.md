...
Don't trust my versions? That's fine lol. Or maybe you are wanting to try making a theme or adding features? Just keep in mind that this gives you the ABSOLUTE latest version, which could have breaking changes. If you want the release snapshot version, there is a source.zip download under each release that is autogenerated by GitHub. Download that, extract it, and skip step 1.

1. Clone the repo and use `git switch v2` to make sure you are on the v2 branch. (this one)
...
7. Once you want to try building an executable (for your platform), run `pnpm tauri build`.
